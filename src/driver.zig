...
// ---- Scaffold proyek web MVC (kerangka Jala) ----

fn tulisProyek(allocator: std.mem.Allocator, root: []const u8, rel: []const u8, isi: []const u8) !void {
    const full = try std.fmt.allocPrint(allocator, "{s}/{s}", .{ root, rel });
    defer allocator.free(full);
    if (std.fs.path.dirname(full)) |d| std.fs.cwd().makePath(d) catch {};
...
const J_APP =
    \\// Titik masuk aplikasi (bootstrap). Jalankan: tenun run app.tenun
    \\impor "jala";
    \\
...
const J_CONFIG =
    \\// Konfigurasi aplikasi (app/config.tenun).
    \\jala_atur("nama_app", "Aplikasi Jala");
;

const J_MODEL =
    \\// Contoh model. Ganti ke modul "orm" untuk basis data (MySQL/Postgres),
    \\// atau pakai penyimpanan kunci-nilai bawaan (simpan/muat).
    \\//
    \\// fungsi pos_semua(): []teks { ... }
;
...
const J_RUTE =
    \\// Definisi rute (mirip routes/web.php). Pola -> fungsi controller.
    \\jala_get("/", home_index);
...
const J_TENTANG =
    \\<p>Aplikasi ini dibuat dengan kerangka kerja <strong>Jala</strong> (MVC) untuk Tenun.</p>
;

const J_CSS =
    \\* { box-sizing: border-box; }
    \\body { font-family: system-ui, sans-serif; max-width: 640px; margin: 2rem auto; padding: 0 1rem; }
    \\nav a { color: #0a7; text-decoration: none; }
...
pub fn baru(allocator: std.mem.Allocator, nama: []const u8) !void {
    ...
    if (std.fs.cwd().access(nama, .{})) {
        try stderr.print("error: folder '{s}' sudah ada\n", .{nama});
        return;
    } else |_| {}

    std.fs.cwd().makePath(nama) catch {};
    ...
    try tulisProyek(allocator, nama, "tenun.json", manifest);
    try tulisProyek(allocator, nama, "app.tenun", J_APP);
    try tulisProyek(allocator, nama, "routes.tenun", J_RUTE);
    try tulisProyek(allocator, nama, "app/config.tenun", J_CONFIG);
    try tulisProyek(allocator, nama, "app/controllers/home.tenun", J_KONTROLER);
    try tulisProyek(allocator, nama, "app/models/example.tenun", J_MODEL);
    try tulisProyek(allocator, nama, "views/layout.batik", J_TATA);
    try tulisProyek(allocator, nama, "views/home.batik", J_BERANDA);
    try tulisProyek(allocator, nama, "views/about.batik", J_TENTANG);
    try tulisProyek(allocator, nama, "views/partials/header.batik", J_PARTIAL);
    try tulisProyek(allocator, nama, "public/style.css", J_CSS);
    try tulisProyek(allocator, nama, ".gitignore", J_GITIGNORE);

    try stdout.print(
        \\[tenun] proyek web '{s}' dibuat.
        \\
        \\Langkah berikut:
        \\  cd {s}
        \\  tenun add jala       (pasang kerangka + web + tampilan)
        \\  tenun run app.tenun  (buka http://localhost:8080)
        \\
        \\Struktur:
        \\  app.tenun             bootstrap
        \\  routes.tenun          definisi rute
        \\  app/
        \\    config.tenun        konfigurasi
        \\    controllers/        controller (logika)
        \\    models/             model (data)
        \\  views/                view .batik (Batik) + partials/
